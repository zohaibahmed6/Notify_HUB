...
## Current step
Step 9 (2026-07-14) — locked spec in `STEP9_PLAN.md` (repo root), a further
...
independently-committed increment at a time. **All 13 increments complete** — see "Step 9
...
viewport — recommend a manual visual pass before treating the redesign screens as fully
...
- [x] P9-06 — SMS History report (skeleton — Expiry Time/PDU Count columns wired in
      P9-07/P9-09) — new `MessagesController`/`GET api/messages`, new
      `OutboundMessage.SentByUsername` column, new `pages/SmsHistoryPage.tsx`. See
      `CODEBASE_MAP.md` §6h. 87/87 fast backend tests passing (2 new). Verified live
...
- [x] P9-07 — message expiry engine — new `MessageStatus.Expired` (7th value), new
      ...
      (not after — see `CODEBASE_MAP.md`'s `MessageDispatcher` row for why). 2 new Domain
...
- [x] P9-08 — reminder SMS engine (retires `ReminderScheduler`/`ReminderWorker`, deleted
      entirely along with `ReminderDueCalculator`/`ReminderTriggerReference` and their
      tests) — new generic event-based engine, `POST api/threads/{id}/reminders`,
      `PATCH`/`POST .../cancel` on `MessagesController`, new `ReminderSmsDialog` modal,
      new Settings → SMS "Reminder SMS defaults" card. See `CODEBASE_MAP.md` §4b for the
      ...
      the real Docker/MySQL stack: create → recompute-on-update → cancel → cancel-again-
      400, full audit trail confirmed, worker logs confirm no more `ReminderWorker` poll
      cycles and the new columns query cleanly.
- [x] P9-09 — PDU segment count — new `PduSegmentCalculator` (GSM-7/UCS-2 segmentation
...
- [x] P9-10 — task forwarding rules — new `TaskForwardingRule` entity/table, new
      ...
      (separate method from the existing Admin-only resolver — `EscalationJob`/status-PATCH
      mass-reassignment deliberately untouched), wired into `ThreadsController.CreateTask`.
      New Settings → Task tab "Task forwarding" card. See `CODEBASE_MAP.md`'s
      ...
      throwaway users: rule-based forward (natural assignee Inactive, valid rule → forwards
...
- [x] P9-11 — recurring task configuration UI (frontend-only, confirmed the backend
      already accepted all 4 fields before starting — no backend changes). New
      ...
      correctly — could not click through the actual React form (no browser tool
      available this session), flagged as with every other UI increment this session.
- [x] P9-12 — user on-leave From/To dates — new `User.LeaveFrom`/`LeaveTo` columns
      ...
      `OnLeave` → `Active` once `LeaveTo` passes (audited). New
