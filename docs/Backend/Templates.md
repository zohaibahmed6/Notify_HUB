...
- `CODEBASE_MAP.md` §3, `TemplatesController` — list (`isActive` filter)/create (defaults
  `IsActive=true`)/update. P9-05: editing a template's `Body` sweeps every `Queued`
  `OutboundMessage` with a matching `TemplateId` and nulls `RenderedBody` (dual-safety net #1;
  `MessageDispatcher`'s unconditional re-render on dispatch is net #2 — see
  `docs/Worker/Dispatcher.md`).
